...
async def main():
    async with async_playwright() as p:
        ...
        print("[1] Page loaded, title:", await page.title())
        
        print("[2] Trying page.evaluate fetch...")
        try:
            ...
            """)
            print("[2] evaluate result:", result)
        except Exception as e:
            print("[2] evaluate error:", e)
        
        print("[3] Trying page.request.post...")
        try:
            response = await page.request.post('https://navi.cnki.net/knavi/journals/searchbaseinfo', {
                'headers': {
                    'Content-Type': 'application/x-www-form-urlencoded',
                    'X-Requested-With': 'XMLHttpRequest',
                },
            ...
            print("[3] status:", response.status)
            text = await response.text()
            print("[3] len:", len(text))
            print("[3] preview:", text[:300])
        except Exception as e:
            print("[3] request error:", e)

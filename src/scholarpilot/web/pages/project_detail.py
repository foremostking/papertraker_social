...
def _clean_markdown(content: str) -> str:
    ...
    1. 将 。。 替换为 。（单句号）
    2. 去除模型思考痕迹（——在笔者看来、——笔者主张等）
    3. 修复 ；。 分隔符（关键词列表中的多余句号）
    4. 在 ### 子标题前换行
    5. 在句号后换行使段落更清晰
    6. 修复 - 开头的列表项换行
    """
    ...
    content = re.sub(r'——从审视实践来看[，,]?这一点值得进一步讨论。*', '', content)
    content = re.sub(r'——从研讨实践来看[，,]?这一点值得进一步讨论。*', '', content)
    content = re.sub(r'——从剖析实践来看[，,]?这一点值得进一步讨论。*', '', content)
    # 通用模式：——XX，这一点值得进一步讨论

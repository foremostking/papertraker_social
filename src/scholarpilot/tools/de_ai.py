...
class DeAIEngine:
    ...
    def strengthen_facts(self, text: str) -> str:
        ...
        return "。".join(rebuilt) + ("。" if not result.endswith("。") else "")
    ...
    def reduce_ai_rate(self, text: str) -> str:
        ...
        for i, s in enumerate(sentences):
            ...
            # 每隔2-3句插入个人评述
            if (i + 1) % 3 == 0 and commentary_idx < 3:
                commentary = _PERSONAL_COMMENTARY[commentary_idx % len(_PERSONAL_COMMENTARY)]
                # 用破折号引出评述
                rebuilt.append(f"——{commentary}这一点值得进一步讨论。")
                commentary_idx += 1

            # 随机为部分句子添加括号补充说明
            if random.random() < 0.15 and len(s) > 15:
                notes = [
                    "（这一点在后续分析中还会涉及）",
                    "（此处需要更多实证支撑）",
                    "（当然这只是一种可能的解释）",
                ]
                s_with_note = s.rstrip("。") + random.choice(notes) + "。"
                rebuilt[-1] = s_with_note
    ...
    def humanize_rewrite(self, text: str) -> str:
        ...
        for i, s in enumerate(sentences):
            ...
            # 每隔3-4句插入思考性表达
            if (i + 1) % 4 == 0 and think_idx < 3:
                expr = _THINKING_EXPRESSIONS[think_idx % len(_THINKING_EXPRESSIONS)]
                rebuilt.append(f"{expr}。")
                think_idx += 1
    ...
    def paraphrase_merge_split(self, text: str) -> str:
        ...
        return "。".join(rebuilt) + ("。" if not text.endswith(("。", "！", "？")) else "")

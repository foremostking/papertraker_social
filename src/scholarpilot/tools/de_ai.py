...
class DeAIEngine:
    ...
    def enrich_details(self, text: str) -> str:
        ...
        # 打破三段式结构：将序数词改为更自然的衔接
        # 注意：只替换论述性序数词（带逗号或句号后缀），
        ...
        # 用正则替换行内无逗号的"一是""二是""三是"（后面直接跟内容，非章节标题）
        # 只在"一是"后面跟着汉字（非"章""节""部分"）时替换
        result = re.sub(r'一是(?!章|节|部分|类|种)', '一方面', result)
        result = re.sub(r'二是(?!章|节|部分|类|种)', '另一方面', result)
        result = re.sub(r'三是(?!章|节|部分|类|种)', '此外', result)
